...
`triage.json` now includes run-level `operationId`, `dryRun`, run-level `guardrails`, dedupe summary fields `duplicateCount` + `dedupeKeysMatched[]`, and per-item `dedupeKey`, `idempotent`, `plannedAction`, and `skippedReasons[]`.
