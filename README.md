...
In `--act` mode, optional mutation guardrails are available: `--require-clean-worktree` (fail fast if git has local changes), `--max-mutations-per-run <n>` (global cap across issue create, branch/commit/push, PR create, comment post, and thread resolve actions), plus `--confirm-label <label>` for `issues watch` (only act on issues that still carry the label).
...
`triage.json` now includes run-level `dryRun`, run-level `guardrails`, and per-item `plannedAction` + `skippedReasons[]`.
